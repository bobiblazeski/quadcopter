...
def dense_block(x, hidden_layer_size, dropout=False,
                batch_normalization=False, leaky_relu=False):
    x = layers.Dense(units=hidden_layer_size)(x)
    if batch_normalization:
        x = layers.BatchNormalization()(x)
    if leaky_relu:
        x = layers.LeakyReLU(alpha=leaky_relu)(x)
    else:
...
class Actor:
    ...
    def __init__(self, state_size, action_size, action_low, action_high, 
                 number_of_layers=3, hidden_layer_size=32,
                 dropout=False, batch_normalization=False, leaky_relu=False):
    ...
    def build_model(self):
        ...
        for _ in range(self.number_of_layers):
            net = dense_block(net, self.hidden_layer_size, self.dropout, 
                              self.batch_normalization, self.leaky_relu)        
...
class Critic:
    ...
    def build_model(self):
        ...
        net_actions = actions
        for _ in range(self.number_of_layers):
            net_states = dense_block(net_states, self.hidden_layer_size, self.dropout, 
                                     self.batch_normalization, self.leaky_relu)
            net_actions = dense_block(net_actions,self.hidden_layer_size, self.dropout, 
                                     self.batch_normalization, self.leaky_relu)                       
        # Try different layer sizes, activations, add batch normalization, regularizers, etc.
...
class DDPG():
    ...
    def __init__(self, task, number_of_layers=3, hidden_layer_size=32, dropout=False,
                 batch_normalization=False, leaky_relu=False,
        ...
        self.actor_local = Actor(self.state_size, self.action_size, self.action_low, self.action_high, 
                                 number_of_layers=number_of_layers, hidden_layer_size=hidden_layer_size, 
        ...
        # Critic (Value) Model
        self.critic_local = Critic(self.state_size, self.action_size, number_of_layers=number_of_layers,
                                   hidden_layer_size=hidden_layer_size, dropout=dropout,
                                   batch_normalization=batch_normalization, leaky_relu=leaky_relu)
        self.critic_target = Critic(self.state_size, self.action_size, number_of_layers=number_of_layers,
                                    hidden_layer_size=hidden_layer_size, dropout=dropout,
                                    batch_normalization=batch_normalization, leaky_relu=leaky_relu)
